...
public class SessionController {
     ...
     */
    @GetMapping("login")
    public ResponseEntity<SessionImpl> login(@RequestHeader("Credentials") String credentials) {
        ...
        if (Pattern.matches(CREDENTIALS_MATCHER, credentials)) {                                                        //Verifica que el String tenga la forma '(email)(:)(password)'
            ...
            return usuarioRepository.findByUsernameAndPassword(data[0], data[1])                                        //Busca el usuario con el email y password
                    .map(usuario -> ResponseEntity.ok(Impl.sessionOf(session.init(usuario))))                           //Si existe, inicializa el usuario con un token publico y privado
